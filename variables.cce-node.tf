variable "nodes" {
  type = map(object({
    ...
    eip_ids                    = optional(set(string))
    eip_count                  = optional(number)
    iptype                     = optional(string)
    bandwidth_size             = optional(number)
    bandwidth_charge_mode      = optional(string)
    sharetype                  = optional(string)
    extend_param_charging_mode = optional(number)
    dedicated_host_id          = optional(string)
    ecs_performance_type       = optional(string)
